...
import MoodStatsWidget from "./MoodStatsWidget";
    ...
    const fetchUpcomingSessions = async () => {
      ...
      try {
        ...
        if (data.success && data.data?.sessions) {
          ...
          const now = new Date();
          const upcoming = (data.data.sessions as Record<string, unknown>[])
            .filter((session) => {
              const sessionDate = new Date(session.scheduledDate as string | Date);
              return session.status === 'APPROVED' && sessionDate >= now;
            })
            .sort((a, b) => {
              const dateA = new Date(a.scheduledDate as string | Date);
              const dateB = new Date(b.scheduledDate as string | Date);
              return dateA.getTime() - dateB.getTime();
            })
            .slice(0, 3) as { id: string; title: string; scheduledDate: string; scheduledTime: string; faculty: { name: string; }; }[]; // Get next 3 upcoming sessions
      ...
      case 'mood':
        return (
          <div className="space-y-6">
            {user?.id === 'demo-student-123' && <MoodStatsWidget isDemoUser={true} />}
            <MoodTracker onScoreUpdate={setWellnessScore} />
          </div>
        );
      case 'chat':
