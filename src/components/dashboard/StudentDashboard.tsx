...
const StudentDashboard = ({ onLogout }: StudentDashboardProps) => {
  ...
  const currentSemester = (user as { currentSemester?: string } | null)
    ?.currentSemester;
    ...
    const checkTodayMoodCheckIn = async () => {
      ...
      try {
        const response = await fetch(
          `/api/mood-checkin/enhanced?studentId=${user.id}&days=1`
        );
  ...
  useEffect(() => {
    const fetchWellnessScore = async () => {
      ...
      try {
        const response = await fetch(
          `/api/wellness-score?studentId=${user.id}`
        );
        const data = await response.json();
  ...
  useEffect(() => {
    ...
    window.addEventListener(
      "dashboard:navigate",
      handleDashboardNavigation as EventListener
    );
    return () =>
        ...
        (error) => {
          ...
          switch (error.code) {
            case error.PERMISSION_DENIED:
              errorMessage =
                "Location access denied. Some safety features may be limited.";
              break;
          ...
          toast({
            ...
            variant:
              locationPermission === "denied" ? "destructive" : "default",
            duration: 5000,
          });
  ...
  const dismissLocationPrompt = () => {
    ...
    toast({
      ...
      description:
        "You can enable location access later in settings for enhanced safety features.",
      duration: 5000,
    });
  };
  ...
  const handleLogout = () => {
    try {
      ...
      toast({
        ...
        description: `Goodbye ${
          user?.name || "Student"
        }! You have been safely logged out. Thank you for using Mann Mitra!`,
        duration: 3000,
      });
  ...
  const quickActions = [
    ...
    },
    {
      ...
      action: () =>
        (window.location.href = `/phq9-survey?studentId=${
          user?.id || "unknown"
        }`),
    },
    {
      ...
      action: () =>
        (window.location.href = `/gad7-survey?studentId=${
          user?.id || "unknown"
        }`),
    },
    {
              ...
              <div className="grid grid-cols-3 gap-4">
                {quickActions.map((action, index) => (
                  ...
                  >
                    ...
                    <div
                      className={`w-14 h-14 rounded-2xl bg-gradient-to-br ${action.color} flex items-center justify-center shadow-md group-hover:shadow-2xl group-hover:scale-110 group-hover:rotate-3 transition-all duration-500 mb-2 relative z-10`}
                    >
                ...
                {isLoadingSessions ? (
                  ...
                  upcomingSessions.map((session, index) => {
                    ...
                    return (
                      ...
                      >
                        <div className="text-2xl transition-transform duration-300 group-hover:scale-110 group-hover:rotate-3">
                          📅
                        </div>
                        <div className="flex-1">
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50/40 via-purple-50/30 to-pink-50/40 relative overflow-hidden">
      ...
      <div
        className="absolute bottom-0 left-0 w-96 h-96 bg-gradient-to-tr from-blue-300/10 to-cyan-300/10 rounded-full blur-3xl animate-pulse pointer-events-none"
          ...
          <Card className="w-full max-w-md rounded-3xl border-0 shadow-2xl animate-in zoom-in-95 slide-in-from-bottom-4 duration-500 bg-white/95 backdrop-blur-xl">
            <CardHeader className="pb-4">
              <CardTitle className="flex items-center text-2xl font-bold">
                <div
                  className="p-3 rounded-2xl bg-gradient-to-br from-blue-100 to-purple-100 mr-3 animate-pulse"
              ...
              <CardDescription className="text-base text-gray-600 mt-2 font-medium">
                Help us provide better safety and emergency support by sharing
                your location.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="bg-gradient-to-r from-blue-50 to-cyan-50 border border-blue-200 rounded-2xl p-5 hover:shadow-md transition-shadow">
                ...
                <ul className="text-sm text-blue-700 space-y-2 font-medium">
                  <li className="flex items-start">
                    ...
                    <span>
                      <strong>Emergency Response:</strong> Quick help in crisis
                      situations
                    </span>
                  </li>
                  <li className="flex items-start">
                    <span className="text-blue-600 mr-2">📍</span>
                    <span>
                      <strong>Nearby Resources:</strong> Find mental health
                      services near you
                    </span>
                  </li>
                  <li className="flex items-start">
                    ...
                    <span>
                      <strong>Safety Features:</strong> Enhanced support when
                      needed
                    </span>
                  </li>
                  <li className="flex items-start">
                    <span className="text-blue-600 mr-2">🏫</span>
                    <span>
                      <strong>Campus Security:</strong> Better student safety
                      monitoring
                    </span>
                  </li>
                </ul>
              ...
              <div className="bg-gradient-to-r from-green-50 to-emerald-50 border border-green-200 rounded-2xl p-4 hover:shadow-md transition-shadow">
                ...
                <p className="text-xs text-green-700 font-medium leading-relaxed">
                  Location data is only used for safety and support purposes.
