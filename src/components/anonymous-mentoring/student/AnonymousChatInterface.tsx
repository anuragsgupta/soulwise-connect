  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        if (result.data.riskLevel === 'HIGH' || result.data.riskLevel === 'CRISIS') {
          toast({
            title: 'Message Sent',
            description: 'Your mentor has been alerted about your message and will respond soon.',
            variant: 'default'
          });
        }
      } else {
  ...
  const handleEndSession = async () => {
    ...
    try {
      ...
      if (result.success) {
        toast({
          title: 'Session Ended',
          description: 'Your chat history has been cleared. Thank you for using anonymous mentoring.'
        });
        router.push('/student/anonymous-mentoring');
      } else {
        toast({
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <CardTitle className="text-xl">You are: {session.student_alias}</CardTitle>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-gray-500 mt-1">
                {session.faculty.jobTitle || session.faculty.facultyType} - {session.faculty.department.name}
              </p>
            </div>
            {isActive && (
              <Button
              ...
              >
                ...
                <p
                  className={`text-xs mt-1 ${
                    message.sender_role === 'STUDENT' ? 'text-purple-200' : 'text-gray-500'
                  }`}
                >
      ...
      <AlertDialog open={showEndDialog} onOpenChange={setShowEndDialog}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Are you sure you want to end this session? Your chat history will be cleared from your view,
              but the mentor will retain the conversation for record-keeping purposes.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
