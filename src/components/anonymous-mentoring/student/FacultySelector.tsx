...
import { MessageSquare, UserCircle, Briefcase, Building2, Filter } from 'lucide-react';
...
export default function FacultySelector() {
  ...
  const applyFilters = () => {
    ...
    if (filterAvailability !== 'ALL') {
      filtered = filtered.filter(f => f.availabilityStatus === filterAvailability);
    }
  ...
  const handleSubmitRequest = async () => {
    ...
    try {
      ...
      if (result.success) {
        toast({
          title: 'Success',
          description: 'Anonymous chat request sent! Waiting for faculty to accept.'
        });
  ...
  return (
    <div className="space-y-6">
      <div className="bg-gradient-to-r from-purple-600 to-indigo-600 text-white p-6 rounded-lg">
        ...
        <p className="text-purple-100">Connect with mentors anonymously for confidential support</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredFaculty.length === 0 ? (
          <div className="col-span-full text-center py-12">
            ...
            <p className="text-gray-600">No faculty members found matching your filters</p>
          </div>
        ) : (
                ...
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    ...
                    <CardDescription className="mt-1">
                      {facultyMember.jobTitle || getFacultyTypeLabel(facultyMember.facultyType)}
                    </CardDescription>
                  </div>
                  <Badge className={getAvailabilityColor(facultyMember.availabilityStatus)}>
      ...
      <Dialog open={showRequestDialog} onOpenChange={setShowRequestDialog}>
        <DialogContent className="sm:max-w-[500px]">
          <DialogHeader>
            ...
            <DialogDescription>
              Send an anonymous request to {selectedFaculty?.name}. Your identity will remain hidden.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4 py-4">
            ...
            <div className="bg-purple-50 border border-purple-200 rounded-lg p-3">
              <p className="text-sm text-purple-800">
                <strong>Note:</strong> Your chat will be completely anonymous. A random name will be assigned to you.
              </p>
            </div>
          </div>
          <DialogFooter>
