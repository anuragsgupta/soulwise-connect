...
export async function POST(
  ...
  try {
    ...
    if (session.status !== 'ACTIVE') {
      return NextResponse.json(
        createResponse(false, 'Session is not active'),
        { status: 400 }
      );
    }
    ...
    } else if (user.userType === 'FACULTY' && session.mentor_id === user.userId) {
      senderRole = 'MENTOR';
    ...
    const crisisKeywords = ['suicide', 'kill myself', 'end my life', 'want to die', 'no reason to live'];
    const highRiskKeywords = ['depressed', 'hopeless', 'worthless', 'give up', 'cant go on'];
