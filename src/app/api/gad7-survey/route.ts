...
import { saveDemoGad7Survey, getDemoGad7Surveys } from '@/lib/demoDB';
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Create survey entry in database for regular users
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Handle demo user with IndexedDB
    if (studentId === 'demo-student-123') {
      const surveys = await getDemoGad7Surveys(studentId);
