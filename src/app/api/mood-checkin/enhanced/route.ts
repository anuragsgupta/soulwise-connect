...
import { addDemoMoodCheckInServer, getDemoMoodCheckInsServer } from '@/lib/demoDataServer';
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (studentId === 'demo-student-123') {
      console.log('✅ Demo student mood check-in');

      if (!moodLevel || moodLevel < 1 || moodLevel > 10) {
        return NextResponse.json(
          createResponse(false, 'Valid mood level (1-10) is required'),
          { status: 400 }
        );
      }
      ...
      addDemoMoodCheckInServer(checkIn);

      return NextResponse.json(
        createResponse(true, 'Mood check-in created successfully', {
          moodCheckIn: checkIn,
          diaryEntry: null,
          isUpdate: false
        }),
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Handle demo student
    if (studentId === 'demo-student-123') {
      console.log('✅ Demo student mood data request for', days, 'days');
      
      try {
        const moodCheckIns = getDemoMoodCheckInsServer(studentId, days);

        // Get today's check-in
        const today = new Date();
        const todayStr = today.toISOString().split('T')[0];
        const todayCheckIn = moodCheckIns.find(m => m.checkInDate === todayStr) || null;
        ...
        console.error('Error getting demo mood data:', error);
        return NextResponse.json(
          createResponse(true, 'Enhanced mood data retrieved successfully', {
            moodCheckIns: [],
            todayCheckIn: null,
