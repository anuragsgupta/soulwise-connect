...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    const verifySession = async () => {
      try {
        ...
        // Check for demo user data from localStorage
        const storedUserData = localStorage.getItem('auth-user');
        const token = localStorage.getItem('auth-token');
        
        // Prepare headers
        const headers: Record<string, string> = {};
        if (token?.startsWith('demo-token-') && storedUserData) {
          headers['x-demo-user'] = encodeURIComponent(storedUserData);
        }
  ...
  useEffect(() => {
    ...
    const refreshInterval = setInterval(async () => {
      try {
        // Check for demo token and user data
        const storedToken = localStorage.getItem('auth-token');
        const storedUserData = localStorage.getItem('auth-user');
        
        const headers: Record<string, string> = {};
        if (storedToken?.startsWith('demo-token-') && storedUserData) {
          headers['x-demo-user'] = encodeURIComponent(storedUserData);
        }
