...
const hasAwsCredentials = () => {
  ...
  return !!(accessKey && secretKey && accessKey.trim() && secretKey.trim());
};
...
const initializeDynamoDBClient = () => {
  ...
  if (!hasAwsCredentials()) {
    console.warn('⚠️ AWS credentials not configured. DynamoDB operations will fail gracefully.');
    return null;
  }
  ...
  docClient = DynamoDBDocumentClient.from(newClient, {
    marshallOptions: {
...
// Get or initialize the document client
const getDocClient = (): DynamoDBDocumentClient | null => {
  return initializeDynamoDBClient();
};
...
export async function saveChatMessage(input: ChatMemoryInput): Promise<ChatMemoryItem> {
  ...
  if (!client) {
    console.warn('⚠️ DynamoDB client not available (missing AWS credentials). Message not saved to database.');
    // Simulate success response even if we couldn't save
    return item;
  }
...
export async function getChatHistory(
  ...
  const client = getDocClient();
  if (!client) {
    console.warn('⚠️ DynamoDB not available. Returning empty chat history.');
    return [];
  }
...
export async function getRecentMessages(
  ...
  const client = getDocClient();
  if (!client) {
    console.warn('⚠️ DynamoDB not available. Returning empty messages.');
    return [];
  }

  return _getRecentMessagesInternal(client, userId, limit);
}

async function _getRecentMessagesInternal(
...
export async function deleteChatHistory(userId: string): Promise<void> {
  const client = getDocClient();
  if (!client) {
    console.warn('⚠️ DynamoDB not available. Cannot delete chat history.');
    return;
  }
